...
interface InventoryItemRepository : JpaRepository<InventoryItem, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT i FROM InventoryItem i WHERE (:location IS NULL OR LOWER(i.location.label) LIKE LOWER(CONCAT('%', :location, '%'))) AND (:profileCode IS NULL OR LOWER(i.profileCode) LIKE LOWER(CONCAT('%', :profileCode, '%'))) AND (:internalColor IS NULL OR LOWER(i.internalColor) LIKE LOWER(CONCAT('%', :internalColor, '%'))) AND (:externalColor IS NULL OR LOWER(i.externalColor) LIKE LOWER(CONCAT('%', :externalColor, '%'))) AND (:coreColor IS NULL OR LOWER(i.coreColor) LIKE LOWER(CONCAT('%', :coreColor, '%'))) AND (:status IS NULL OR i.status = :status)")
    fun findFiltered(location: String?, profileCode: String?, internalColor: String?, externalColor: String?, coreColor: String?, status: ItemStatus?, pageable: org.springframework.data.domain.Pageable): org.springframework.data.domain.Page<InventoryItem>
}
